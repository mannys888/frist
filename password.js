...
function detail(tid) {
 ...
 // 处理虚拟键盘按键
  if (unlockMode && tid.startsWith('__UNLOCK_KEY__')) {
    ...
    let videoList = [];
    // 远程JSON地址（可配置在 __ext_config.global.unlockVideosUrl）
    ...
    try {
        ...
        print("获取远程视频列表失败，使用默认: " + e.message);
        videoList = [{ title: "默认庆祝视频", url: "https://vd2.bdstatic.com/mda-sbehdejw4kmibhkh/576p/h264/1771157811027978795/mda-sbehdejw4kmibhkh.mp4" }];
    }
